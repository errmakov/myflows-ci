...
app.post('/githubhook/push', (req, res) => {
    try {
        if (req.body.ref.includes('stage')) {
            ...
            execSync(`cd ${process.env.DEPLOY_DIR} && echo '<a href="${req.body.head_commit.url}">${req.body.head_commit.id}</a> at  ${req.body.head_commit.timestamp} by ${req.body.pusher.name}'> public/version.txt`);
