...
app.post('/githubhook/push', (req, res) => {
    try {
        if (req.body.ref.includes('stage')) {
            ...
            execSync(`cd ${process.env.DEPLOY_DIR} && echo '${req.body.head_commit.url}///${req.body.head_commit.id}</a>///${req.body.head_commit.timestamp}///${req.body.pusher.name}'> public/version.txt`);
